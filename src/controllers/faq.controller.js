...
const addFAQs = async (req, res) => {
  try {
    ...
    const faqs = await FAQService.addFAQs(payload.faqs, payload.cityId);
    res.status(201).json({ status: 'success', message: 'FAQs added', response: faqs });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
  }
};
const genearteFAQ = async (req, res) => {
  try {
    const { cityName } = req.body;
    const faqs = await FAQService.generateFAQ(cityName);
    res.json({ status: 'success', message: 'FAQs generated', response: faqs });
  } catch (error) {
    res.status(500).json({ status: 'error', message: error.message });
  }
};
